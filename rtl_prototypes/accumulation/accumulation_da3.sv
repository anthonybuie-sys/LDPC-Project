...
module accumulation_da3 #(
  ...
  always @(posedge clk) begin
    if (rst) begin
      ...
      for (lane = 0; lane < LANES; lane = lane + 1) begin
        mag_a = (edge_id_a_i == old_imin_i[lane*EDGE_ID_W +: EDGE_ID_W]) ?
          old_min2_i[lane*MSG_W +: MSG_W] : old_min1_i[lane*MSG_W +: MSG_W];
        mag_b = (edge_id_b_i == old_imin_i[lane*EDGE_ID_W +: EDGE_ID_W]) ?
          old_min2_i[lane*MSG_W +: MSG_W] : old_min1_i[lane*MSG_W +: MSG_W];
        c2v_a_s0[lane*(MSG_W+1) +: (MSG_W+1)] <=
          signed_c2v(mag_a, old_total_sign_i[lane] ^ old_q_sign_a_i[lane]);
        c2v_b_s0[lane*(MSG_W+1) +: (MSG_W+1)] <=
          signed_c2v(mag_b, old_total_sign_i[lane] ^ old_q_sign_b_i[lane]);

        q_tmp = $signed(app_a_perm_s0[lane*APP_W +: APP_W]) -
          $signed(c2v_a_s0[lane*(MSG_W+1) +: (MSG_W+1)]);
        q_a_s1[lane*Q_W +: Q_W] <= sat_q(q_tmp);
        q_tmp = $signed(app_b_perm_s0[lane*APP_W +: APP_W]) -
          $signed(c2v_b_s0[lane*(MSG_W+1) +: (MSG_W+1)]);
        q_b_s1[lane*Q_W +: Q_W] <= sat_q(q_tmp);

        min1_tmp = old_min1_s0[lane*MSG_W +: MSG_W];
        min2_tmp = old_min2_s0[lane*MSG_W +: MSG_W];
        imin_tmp = old_imin_s0[lane*EDGE_ID_W +: EDGE_ID_W];
        abs_a = abs_q(q_a_s1[lane*Q_W +: Q_W]);
        abs_b = abs_q(q_b_s1[lane*Q_W +: Q_W]);
        if ((abs_a < min1_tmp) || ((abs_a == min1_tmp) && (edge_id_a_s0 < imin_tmp))) begin
          min2_tmp = min1_tmp;
          min1_tmp = abs_a;
          imin_tmp = edge_id_a_s0;
        end else if (abs_a < min2_tmp) begin
          min2_tmp = abs_a;
        end
        if ((abs_b < min1_tmp) || ((abs_b == min1_tmp) && (edge_id_b_s0 < imin_tmp))) begin
          min2_tmp = min1_tmp;
          min1_tmp = abs_b;
          imin_tmp = edge_id_b_s0;
        end else if (abs_b < min2_tmp) begin
          min2_tmp = abs_b;
        end
        q_a_o[lane*Q_W +: Q_W] <= q_a_s1[lane*Q_W +: Q_W];
        q_b_o[lane*Q_W +: Q_W] <= q_b_s1[lane*Q_W +: Q_W];
        new_min1_o[lane*MSG_W +: MSG_W] <= min1_tmp;
        new_min2_o[lane*MSG_W +: MSG_W] <= min2_tmp;
        new_imin_o[lane*EDGE_ID_W +: EDGE_ID_W] <= imin_tmp;
        new_total_sign_o[lane] <= old_total_sign_s0[lane] ^
          q_a_s1[lane*Q_W + Q_W - 1] ^ q_b_s1[lane*Q_W + Q_W - 1];
      end
    end
  end
endmodule
